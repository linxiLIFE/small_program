...
async function persist(table, id, data, account) {
  const existing = await getOptional(table, id);
  const now = Date.now();
  ...
  await db.collection(table).doc(id).set({ data: record });
  const auditId = `audit-${crypto.randomUUID()}`;
  await db.collection(COLLECTIONS.auditLogs).doc(auditId).set({ data: { id: auditId, operatorId: account.uid || account.openid, operatorRole: account.role, action: 'SAVE_CATALOG', objectType: table, objectId: id, createdAt: now } });
  return record;
}
async function saveCategory(payload = {}) {
  const { account } = await requireRole(['OWNER']);
  const record = await persist(COLLECTIONS.categories, idOf(payload, 'cat'), { name: nameOf(payload.name, '分类名称'), icon: ['✦','⌁','◌','♡','✿','◇'].includes(payload.icon) ? payload.icon : '✦', color: /^#[0-9a-f]{6}$/i.test(payload.color || '') ? payload.color : '#f1ded8', coverUrl: imageOf(payload, 'coverUrl', true), enabled: payload.enabled !== false, sort: integer(payload.sort || 0, '排序') }, account);
...
async function saveWork(payload = {}) {
  ...
  assert(service && service.enabled !== false, 'INVALID_SERVICE', '请选择已上架的小项目');
  const category = await getOptional(COLLECTIONS.categories, service.categoryId);
  assert(category && category.enabled !== false, 'INVALID_CATEGORY', '所属大类已停用');
  const record = await persist(COLLECTIONS.works, idOf(payload, 'work'), { title: nameOf(payload.title, '款式名称'), imageUrl: imageOf(payload, 'imageUrl'), serviceId: service.id || service._id, categoryId: category.id || category._id, categoryName: category.name, published: payload.published !== false, featured: payload.featured === true, featuredSort: integer(payload.featuredSort || 0, '精选排序'), sort: integer(payload.sort || 0, '排序') }, account);
...
async function saveTechnician(payload = {}) {
  ...
  const record = await persist(COLLECTIONS.technicians, idOf(payload, 'tech'), { name: nameOf(payload.name, '技师姓名'), title: String(payload.title || '').slice(0,80), bio: String(payload.bio || '').slice(0,500), avatarUrl: imageOf(payload,'avatarUrl',true), skills, sort: integer(payload.sort || 0,'排序'), enabled: payload.enabled !== false }, account);
...
async function listCatalog() {
  ...
  return {
    ...
    works: works.filter(item => !item.archived).map(item => ({ ...publicWork(item), published: item.published !== false, sort: item.sort || 0 })),
