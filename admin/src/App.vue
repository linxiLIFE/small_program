      ...
      <template v-else>
        ...
        <section v-else-if="page === 'settings'" class="page-section">
          ...
          <section class="panel settings-panel">
            ...
            <div class="settings-block"><div class="settings-block-heading"><h2>微信订阅提醒</h2><span>模板字段名需与公众平台模板一致</span></div><label class="switch-row"><span>启用订阅消息发送</span><input v-model="settings.notifications.enabled" type="checkbox"/></label><div class="form-grid notification-settings"><label>到店提醒提前分钟<input v-model.number="settings.notifications.arrivalLeadMinutes" type="number" min="15" max="10080"/></label><div v-for="(item,key) in settings.notifications.templates" :key="key" class="notification-template-card"><strong>{{({appointmentSuccess:'预约成功',arrivalReminder:'提醒到店',checkInSuccess:'核销成功',noShowRefund:'退款成功'} as Record<string,string>)[key]}}</strong><label>模板 ID<input v-model.trim="item.templateId" placeholder="公众平台模板 ID"/></label><div class="template-key-grid"><label>项目字段<input v-model.trim="item.serviceKey" placeholder="thing1"/></label><label v-if="item.timeKey">时间字段<input v-model.trim="item.timeKey" placeholder="date2 / time1"/></label><label v-if="item.technicianKey">技师字段<input v-model.trim="item.technicianKey" placeholder="thing19"/></label><label v-if="item.addressKey">门店字段<input v-model.trim="item.addressKey" placeholder="thing7"/></label><label v-if="item.storeKey">门店字段<input v-model.trim="item.storeKey" placeholder="thing3"/></label><label v-if="item.amountKey">金额字段<input v-model.trim="item.amountKey" placeholder="amount6"/></label><label v-if="item.statusKey">状态字段<input v-model.trim="item.statusKey" placeholder="phrase3"/></label></div></div></div><p class="form-help">模板 ID 和字段名可在微信公众平台「订阅消息 / 我的模板」复制。当前已按模板详情配置关键词；保存后才会发布到线上。</p></div>
