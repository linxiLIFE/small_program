...
function refundStatusLabel(status?: string): string {
  return ({ INIT: '退款待提交', PENDING_CONFIG: '退款待配置', SUBMITTING: '退款提交中', PROCESSING: '退款处理中', SUCCESS: '退款已到账', RETRY_REQUIRED: '退款需重新发起', MANUAL_ACTION: '退款需人工处理', CLOSED: '退款已关闭', ABNORMAL: '退款异常' } as Record<string, string>)[status || ''] || '';
}

function canRequestRefund(order: AdminOrder): boolean {
  return ['RESERVED', 'ARRIVED', 'IN_SERVICE', 'COMPLETED', 'CANCEL_PENDING_REFUND'].includes(order.status)
    && ['NOT_REQUIRED', 'RETRY_REQUIRED', 'CLOSED'].includes(order.refundStatus || 'NOT_REQUIRED');
}
