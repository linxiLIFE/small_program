...
<template><section class="page-section"><div class="page-intro"><h1>首页宣传</h1><button class="primary-button" :disabled="banners.length>=20||busy||uploads>0" @click="add">＋ 添加宣传图</button></div><div class="banner-editor-grid"><article v-for="(banner,index) in banners" :key="banner.id" class="panel banner-editor"><ImageUploader v-model="banner.imageUrl" :preview-url="banner.previewUrl" label="上传横版宣传图" @busy="uploads+=$event?1:-1"/><footer><span>{{index+1}} / {{banners.length}}</span><div><button class="soft-button" :disabled="index===0||busy||uploads>0" @click="move(index,-1)">前移</button><button class="soft-button" :disabled="index===banners.length-1||busy||uploads>0" @click="move(index,1)">后移</button><button class="text-button" :disabled="busy||uploads>0" @click="banners.splice(index,1)">移除</button></div></footer></article><button class="add-tile banner-add" :disabled="banners.length>=20||busy||uploads>0" @click="add"><span>＋</span>添加宣传图</button></div><p class="field-error" v-if="error" role="alert">{{error}}</p><p class="inline-success" v-if="notice" role="status">{{notice}}</p><div class="settings-actions"><button class="primary-button" :disabled="busy||uploads>0" @click="save">{{busy?'保存中…':'保存宣传图'}}</button></div></section></template>
